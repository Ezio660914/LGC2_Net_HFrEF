...
from source.data.load_data import load_data_cross_validate_train_test_four_locations_parallel_external_validation_data
...
data_dir = os.path.join(project_dir, "dataset/HFrEF_community_scenario_test_set")
...
log_dir = os.path.join(project_dir, "logs/community_scenario_test_set")
...
for i in range(k_folds):
    ECG_PCG_test_x, ECG_test_y, ECG_test_record_label, ECG_test_num_list, ECG_test_patch_name_list, ECG_test_recording_name_list = \
        load_data_cross_validate_train_test_four_locations_parallel_external_validation_data(data_dir, resample_frequency, EF_threshold)
    ...
    while True:
        ...
        floder_pos = pred_array_test[Y_test == 1]
        floder_pos_list.append(floder_pos.shape[0])
        floder_neg = pred_array_test[Y_test == 0]
        floder_neg_list.append(floder_neg.shape[0])
        TP_pred = floder_pos[floder_pos == 1]
        TP_list.append(TP_pred.shape[0])
        TN_pred = floder_neg[floder_neg == 0]
    ...
    floder_pos_array = np.hstack(floder_pos_list).reshape(-1, 1)
    floder_neg_array = np.hstack(floder_neg_list).reshape(-1, 1)
    TP_array = np.hstack(TP_list).reshape(-1, 1)
    TN_array = np.hstack(TN_list).reshape(-1, 1)
    acc_array = (TP_array + TN_array) / (floder_pos_array + floder_neg_array)
    Sen_array = TP_array / floder_pos_array
    Spe_array = TN_array / floder_neg_array
    ...
    if i == 0:
        floder_pos_array_all = floder_pos_array
        floder_neg_array_all = floder_neg_array
        TP_array_all = TP_array
        TN_array_all = TN_array
    else:
        floder_pos_array_all = np.concatenate((floder_pos_array_all, floder_pos_array), axis=1)
        floder_neg_array_all = np.concatenate((floder_neg_array_all, floder_neg_array), axis=1)
...
floder_pos_array_all = floder_pos_array_all.sum(axis=1).reshape(-1, 1)
floder_neg_array_all = floder_neg_array_all.sum(axis=1).reshape(-1, 1)
TP_array_all = TP_array_all.sum(axis=1).reshape(-1, 1)
TN_array_all = TN_array_all.sum(axis=1).reshape(-1, 1)
Acc_ave = (TN_array_all + TP_array_all) / (floder_neg_array_all + floder_pos_array_all)
Sen_ave = TP_array_all / floder_pos_array_all
Spe_ave = TN_array_all / floder_neg_array_all
